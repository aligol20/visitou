...
export default class Final extends Component {
    ...
    setOrder() {
        ...
        try {
            AsyncStorage.getItem('userInfo', (err, store) => {
                ...
                let dd = JSON.parse(store)[0].job;
                let xx = JSON.parse(store)[0].workPhone;
                let zz = JSON.parse(store)[0].name +' '+ JSON.parse(store)[0].family;
                console.log(zz, 'mahnazparivash27');
                this.setState({orderPhoneM: rr,job:dd,workPhone:xx,tooleSag:zz})
                    ...
                    stores.map((result, i, store) => {
                        ...
                        if (key.includes('order')) {
                            ...
                            let gooz = {
                                ...
                                ordererName: this.state.tooleSag


                            };
                            moz.push(gooz);
                            console.log(moz, 'cccccccccc');
                            console.log(this.state.shokhm, 'jfjdsfjd1378');
                            console.log(moz, 'jfjdsfjd13789');
                            this.setState({shokhm: moz});
                            console.log(this.state.shokhm, 'jfjdsfjd137890');
    ...
    sendToServer() {
        ...
        for (let i=0;i<order.length;i++){
            if(!ko.map(function (t) {
                    return t.productProvider
                }).includes(order[i].productProvider)){
                ko.push(order[i])
            }
        }
        console.log(ko, '1q1q1q13');
        for(let i=0;i<ko.length;i++){
            let aaaan=  {
                "to" : "/topics/"+ko[i].productProvider,
                "priority" : "high",
                "notification" : {
                    "body" : "شما سفارش دارید",
                    "title" : "سلام!",
                }
            };
        ...
        const url = 'http://visitou.ir/api/oriori.php';
            ...
            fetch(url, {
                ...
                body: JSON.stringify(order)
                        ...
                        console.log(response, 'fjdhfjkdfhdfkdkfh');
                AsyncStorage.getAllKeys((err, keys) => {
                    // let r = JSON.parse(keys);
                    let ghoo = keys.filter(x => x.includes('order'));
                    console.log(ghoo,'jfjdsfjd1');
                    console.log(keys,'jfjdsfjd');
            ...
            AsyncStorage.multiGet(keys, (err, stores) => {
                ...
                stores.map((result, i, store) => {
                    ...
                    if (key.includes('order')) {
                        ...
                        console.log(listArray, 'jfjdsfjd');
                    ...
                    aan.sort(function(a, b){return parseInt(a.provider.provider_details_id) - parseInt(b.provider.provider_details_id)});
    ...
    whichGel(id){
        ...
        if(this.state.dataraw.length-1>=r) {
            ...
            if (a.provider.provider_details_id === b.provider.provider_details_id) {
        ...
        const hth=this.state.dataraw[id].provider.provider_details_id;
        let ghoo = sis.filter(x => x.provider.provider_details_id === hth );
        let hyh=0;
        ...
        console.log(hth,'gqgqgqgqgq44444');
            ...
            <Root>
                <Content>
                    ...
                    <View style={{
                        ...
                        />
                            <Icon9 style={{margin: 10}} name="location-pin" color="#2ECC71" size={30}
                            />
                        <Icon style={{marginRight: 10, marginLeft: 10}} name="arrow-right" color="#F39C12" size={30}
                        />
                            <Icon5 style={{margin: 10}} name="check" color="#F39C12" size={30}
                            />
                    ...
                    <ScrollView>
                        ...
                        <View style={{
                            ...
                            <View style={{width: width,backgroundColor:'white', borderRadius: 7, margin: 5,
                                ...
                                <TextInput style={{fontFamily:'B Koodak',color:'#34495E',backgroundColor:'white'
