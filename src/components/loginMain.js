...
export default class LoginMain extends Component {
    ...
    sendSms(response,ph){
        ...
        if (aan !== 1) {
            fetch( "https://api.kavenegar.com/v1/" +
                ...
                "receptor="+y+"&" +
                    "token="+aan+"&" +
                    "template=visitouVerify").then((response) =>console.log(response, 'mahnazparivash222'));
    ...
    sendToServer() {
        if(this.state.verfiCode.length===5) {
            ...
            AsyncStorage.getItem('userPhoneNumber', (err, store) => {
                ...
                let yuy = [{passCode: this.state.verfiCode, phoneNumber: JSON.parse(store)}];
                // yuy.push({passCode: this.state.verfiCode,phoneNumber: store});
                try {
                    fetch('http://visitou.ir/api/login.php', {
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json',
    ...
    finalProcesses(rrr) {
        ...
        console.log(userData[0], 'mahnazparivash6');
        console.log('ckjjkjjk', 'mahnazparivash7');
        if (userData[0].phonenumber !== null) {
            let userInfo = [];
            userInfo.push({name: userData[0].name, family: userData[0].family, phone: userData[0].phonenumber,job:userData[0].job,workPhone:userData[0].work_phone})
